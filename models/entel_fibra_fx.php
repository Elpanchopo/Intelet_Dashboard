...
function getGestionesEjecutivosHora3() {
    $query = "SELECT * FROM v_grx_gestionejehora_fibra order by 1";
    echo getJsonData($query, 3);
}

function getTipoGestionesEjecutivos3() {
    $query = "SELECT * FROM v_tabla_tipogestion_ejec_diario_totales_fibra order by 1";
    echo getJsonData($query, 3);
}

//Funciones Tablas
function getTablaGestionesEjecutivosHora3(){
    $query = "SELECT * FROM v_gest_ejec_hora_fibra";
    return getJsonData($query, 3);
}

function getTablaTGDiarioTotal3(){
    $query = "SELECT * FROM v_tabla_tipogestion_ejec_diario_fibra";
    return getJsonData($query, 3);
};

function getTablaTGDiarioSumTotal3(){
    $query = "SELECT sum(ct) as ct, sum(te) as te, sum(nc) as nc, sum(cp) as cp, sum(total) as total from v_tabla_tipogestion_ejec_diario_fibra";
    return getJsonData($query, 3);
}

// Funcion que consulta la vista con los valores de las tarjetas y retorna un ARRAY para asignarlo en la vista
function getCardsData() {
    $query = "SELECT * FROM v_cards_test_fibra";
